...
function buildMatch(plan, index) {
  ...
  for (let rn = 0; rn < total; rn++) {
    ...
    // Plant is a mid-round event — independent of who is alive at round end (an attacker
    // planted earlier, even if the defenders then retook). This is what makes defuses possible.
    const allPuuids = [...blue, ...red].map((p) => p.puuid);
    const didPlant = chance(0.62);
    if (didPlant) {
      plantSite = pick(SITES[map]);
      bombPlanter = pick(sideOfTeam[attackingSide].map((p) => p.puuid));
      plantRoundTime = ri(25000, 50000);
      plantLocation = locAt(map, plantSite, 0.05);
      plantPlayerLocations = shuffle(allPuuids).slice(0, ri(2, 4))
        .map((p) => ({ puuid: p, viewRadians: +(rnd() * 6.283).toFixed(3), location: locAt(map, plantSite, 0.18) }));
      // Defender retake: if the defending side won a planted round, they defused it.
      if (winner === defendingSide && chance(0.8)) {
